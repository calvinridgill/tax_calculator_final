...
(async () => {
  ...
  const customData = [
    ["Custom Data 1", "Value 1"],
    ["Custom Data 2", "Value 2"],
    // Add more rows as needed
  ];
  await googleSheet.addCustomDataToOriginalSheet(customData);

  // Copy content to a new sheet
  const newSpreadsheetUrl = await googleSheet.copyTaxCalculatorContent("newUser@example.com");
